<div class="content my-[60px]">
    <div class="category w-[90%] lg:w-[80%] mx-auto">
        <ul class="show_pc w-full lg:flex">
            <li
                class="w-[135px] font-semibold  py-3  text-center {{ Request::get('category_name') == 'all' || Request::get('category_name') == '' ? 'text-white bg-[#016532] rounded-[4px]' : 'text-[#667085] border-r-[1px] border-[#01653280]' }}">
                <a href="list_course?category_name=all">{{ __('all') }}</a>
            </li>
            @foreach ($category_courses as $category)
                <li
                    class=" w-[135px] font-semibold py-3  text-center {{ Request::get('category_name') == $category->slug ? 'text-white bg-[#016532] rounded-[4px]' : 'text-[#667085] border-r-[1px] border-[#01653280]' }}">
                    <a href="list_course?category_name={{ $category->slug }}">{{ __($category->slug) }}</a>
                </li>
            @endforeach
        </ul>
        <div class="show_mobile">
            <select id="catogory" onchange="location = this.value;" class="bg-white border-[2px] border-[#016532] font-semibold text-[#016532] text-sm rounded-lg  block w-full p-2.5 ">
                ...
                @foreach ($category_courses as $category)
                <option class="{{ Request::get('category_name') == $category->slug  ? 'bg-[#016532] text-white' : ''}}" {{ Request::get('category_name') == $category->slug  ? 'selected' : ''}} value="list_course?category_name={{ $category->slug }}"><a href="list_course?category_name={{ $category->slug }}">{{ __($category->slug) }}</a> </option>
                @endforeach
            </select>
        </div>
