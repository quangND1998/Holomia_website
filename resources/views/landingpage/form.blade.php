<div id="form_advise" class="containt6 bg-[#FDF7F7] {{ Request::path() === 'contact' ||  Request::path() === 'thanks_register' ? 'hidden' : ''  }}">
    <div class="lg:w-[80%] title_center  mx-auto lg:flex py-[60px]">
        ...
        <div class="lg:w-1/2 mx-12 relative my-auto ">
            ...
            {{ csrf_field() }}
                <div class="my-3">
                    <label for="name" class="font-semibold !text-left">{{ __('hoten') }}</label>
                    <input id="name" type="text" name="name" class="w-full border border-[#CBD5E0] bg-[#FDF7F7] rounded-[10px] py-2" required>
                </div>
                <div class="my-3">
                    <label for="old" class="font-semibold !text-left">{{ __('tuoi') }}</label>
                    <input id="old" type="text" name="old" class="w-full border border-[#CBD5E0] bg-[#FDF7F7] rounded-[10px] py-2" required>
                </div>
                <div class="my-3">
                    <label for="phone" class="font-semibold !text-left">{{ __('phone') }}</label>
