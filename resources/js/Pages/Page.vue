      ...
      <span v-if="disabled">This feature is disabled!</span>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            __("action")
                ...
                <draggable v-model="pages" tag="tbody" @change="onUnpublishedChange" v-bind="dragOptions"
                    @start="isDragging = true" @end="isDragging = false" item-key="id_priority">
                    <template>
                        <tr v-for="(element, index) in pages" :key="index"
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row"
                                class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">{{ index
            + 1 }}</th>
                            <td class="px-6 py-4">
              ...
              >
                <icon name="list" />
                            ...
                            <td class="px-6 py-4">
                                ...
                                <Link :href="route('section.index', element.id)" data-toggle="tooltip"
                                    data-placement="bottom" title="Sections"
                                    class="inline-block px-4 py-1.5 bg-gray-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-gray-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">
                                <icon name="secion" />
                                </Link>
